...
if selected == 'Home':
    ...
    # Add some information about the available predictions
    ...
    with col2:
        ...
        st.markdown(''':droplet: **Chronic Kidney Disease**  
        Assess your risk of chronic kidney disease''') 
    
    # with col3:
    #     st.markdown(''':stroke: **Stroke**  
    #     Predict your risk of stroke''') 
    
        # st.markdown(''':kidney: **Chronic Kidney Disease**  
        # Assess your risk of chronic kidney disease''') 
# Initialize models
diabetes_model = None
...
if os.path.exists('parkinsons.csv'):
    try:
        ...
        if missing_columns:
            ...
            # Use all numeric columns except 'status' as features
...
if selected == "Parkinsons Prediction":
    ...
    # Display input fields in 3 columns for better UI
    col1, col2, col3 = st.columns(3)
...
# Check if lung cancer data file exists
...
if os.path.exists(lung_cancer_file):
    try:
        ...
        # Check required columns - updated to match the actual CSV structure
        ...
        if missing_columns:
            ...
            # Drop non-numeric columns we don't need for the model
            ...
            for col in lung_cancer_data.columns:
                if col != 'Result':  # Skip target column
    ...
    try:
        ...
        if 'EventCKD35' not in ckd_data.columns:
            ...
            for col in ckd_data.columns:
                if col != 'EventCKD35':  # Skip target column
...
if os.path.exists('stroke-data.csv'):
    try:
        ...
        if missing_columns:
            ...
            # Fix for pandas chained assignment warning
    ...
    with st.form("lung_cancer_form"):
        ...
        if submitted:
            with st.spinner('Analyzing your information...'):
                try:
                    # Prepare input data with the available features
                    input_data = {
                        'Age': [age],
                        'Smokes': [smokes],
                        'AreaQ': [areaq],
                    ...
                    # Fill in the values from input_data, matching the feature names exactly
                    for feature in lung_cancer_features:
                        # Find matching input data (case-insensitive)
                        matching_input = next((k for k in input_data.keys() if k.lower() == feature.lower()), None)
                        if matching_input:
                            input_df[feature] = input_data[matching_input]
                        else:
                            # If feature not in input data, use 0 as default
                            input_df[feature] = 0
                    
                    # Ensure all columns are numeric and handle any NaN values
                    ...
                    # Calculate risk level and get recommendations
                    ...
                    # Display risk level with color-coded header
                    st.markdown(f"""
        ...
        with st.form("stroke_form"):
            ...
            if submitted:
                try:
                    ...
                    # Ensure all columns are numeric and fill any remaining NaN values with 0
                    input_df = input_df.apply(pd.to_numeric, errors='coerce').fillna(0)
...
if selected == "Chronic Kidney Disease Prediction":
    ...
    if st.button("Assess CKD Risk", type="primary", use_container_width=True):
        try:
            ...
            # Add additional features with default values if they exist in the model
            additional_features = {
                'smoking_status': [0],
                'alcohol_intake': [0],
                'ever_married': [0],
            ...
            # Merge the input data with additional features
